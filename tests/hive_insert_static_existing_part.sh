...
procuder() {
  echo "$(get_ts) Impala> Resetting the partition to have only one row"
  $IMPALA_EXEC "insert overwrite table $DB.tbl1 partition (p=0) select $COLUMNS from $DB.tbl3 where p=0"
  echo "$(get_ts) Hive> Adding a new row to the existing partition so it has 2 rows"
  $HIVE_EXEC "set hive.stats.autogather=false; insert into $DB.tbl1 partition(p=0) select $COLUMNS from $DB.tbl3 where p=0"
}
