...
procuder() {
  ...
  $IMPALA_EXEC "insert into $DB.$TBL partition(p) select $COLS,cast(p+200 as int) from default.src_tbl where p<200"
  echo "$(get_ts) Hive> Dynamically overwriting 200 partitions"
  $HIVE_EXEC "set hive.stats.autogather=false; set hive.exec.max.dynamic.partitions.pernode=200; insert overwrite table $DB.$TBL select $COLS, p+200 from default.src_tbl where p<200"
}
